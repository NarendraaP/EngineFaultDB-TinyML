...
\setlength{\textfloatsep}{3.5pt plus 0.5pt minus 0.5pt}
\setlength{\floatsep}{3.0pt plus 0.5pt minus 0.5pt}
\setlength{\intextsep}{3.0pt plus 0.5pt minus 0.5pt}
\setlength{\dbltextfloatsep}{3.5pt plus 0.5pt minus 0.5pt}
\setlength{\dblfloatsep}{3.0pt plus 0.5pt minus 0.5pt}
...
\begin{figure}[t]
\centering
\includegraphics[width=0.68\columnwidth]{physical_pipeline.png}
...
\begin{figure}[t]
\centering
\includegraphics[width=0.49\columnwidth]{latency_distributions.png}\hfill
\includegraphics[width=0.49\columnwidth]{params_vs_latency.png}
...
\begin{figure}[t]
\centering
\includegraphics[width=0.88\columnwidth]{host_vs_esp32.png}
...
\textbf{Flash Footprint:} Complete firmware occupies $330,153\,\text{B}$ ($25.19\%$ of the $1.25\,\text{MB}$ partition, $7.87\%$ of Flash). All four models combined require $<15\,\text{KB}$ ROM.
\textbf{Static SRAM:} Global data and stacks require $61,944\,\text{B}$ ($18.90\%$ of $320\,\text{KB}$ on-chip SRAM).
\textbf{Tensor Arena:} The runtime allocator committed exactly $\mathbf{916\,\text{B}}$ within the $8,192\,\text{B}$ arena ($\mathbf{88.82\%}$ unallocated headroom).
\textbf{Heap Stability:} Free heap remained constant at $237,452\,\text{B}$ across all $25,200$ executions (\textbf{0 Bytes dynamic allocation, zero leakage}).
...
\textbf{1) Single Silicon Target:} Evaluated on dual-core Xtensa LX6 ESP32-D0WD-V3; findings do not directly generalize across other ISAs (ARM Cortex-M, RISC-V) or vector variants (ESP32-S3).
\textbf{2) Model Topology Focus:} Evaluations focused on sub-4 KB MLPs; convolutional and recurrent models exhibit different memory dynamics.
\textbf{3) Software Stack:} Benchmarks utilize portable reference C++ integer kernels in TFLM; assembly-optimized kernels (e.g., ESP-NN) may achieve lower latencies.
\textbf{4) Energy Profiling:} We characterize latency and memory; power dissipation was not instrumented via current shunts.
\textbf{5) Empirical Latency vs. Formal WCET:} Maximum observed latency ($102\,\si{\micro\second}$) reflects empirical distributions, not a formal static WCET bound.
\textbf{6) Isolated Kernel Scope:} Timings capture pure inference, excluding physical sensor ADC conversion and CAN-bus scheduling delays.
