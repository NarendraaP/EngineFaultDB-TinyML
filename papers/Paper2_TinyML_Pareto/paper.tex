...
Figure~\ref{fig:accuracy_vs_macs} and Figure~\ref{fig:accuracy_vs_model_size} illustrate the core trade-off: a model with minimal parameters may underfit the feature manifold, whereas heavily pruned dense models may reduce arithmetic operations without freeing Flash memory. Motivated by the need for clear empirical guidance, we formulate a multi-objective Pareto analysis to determine which compression paradigms yield non-dominated operating points for ultra-low-resource sensor diagnostics.
...
\begin{itemize}
    ...
    \item \textbf{When MCU execution latency is the primary concern:} Host latency measurements are insufficient; physical on-device profiling is required to measure hardware cycle counts and memory bus contention.
\end{itemize}
...
\begin{enumerate}
    ...
    \item \textbf{Host Timing Limitations:} Latency was profiled on an x86\_64 CPU and does not establish microcontroller Worst-Case Execution Time (WCET).
    \item \textbf{Absence of Physical Energy Profiling:} We report theoretical active MACs; physical power dissipation on silicon was not measured.
...
All experimental code, trained model FlatBuffers, dataset splits, and verification scripts are available in the project repository. The model metrics can be verified by executing \path{scripts/phase4_5_verification.py} against \path{results/tinyml_model_profile_verified.csv}.
