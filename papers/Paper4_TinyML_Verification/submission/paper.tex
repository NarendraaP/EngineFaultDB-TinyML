...
\begin{abstract}
The deployment of deep learning models on resource-constrained microcontrollers (TinyML) is rapidly expanding, yet empirical evaluation in the field suffers from critical methodological vulnerabilities. Training-time in-memory metrics frequently diverge from disk-serialized binary behavior; theoretical weight sparsity is routinely conflated with physical storage compression; and host simulation timings are improperly extrapolated to microcontroller real-time guarantees. In this paper, we propose a multi-level \textbf{TinyML Scientific Verification Framework} spanning seven core dimensions: Data Isolation, Serialized Binary Integrity, Quantization Graph Inspection, Sparsity and Multiply-Accumulate (MAC) Accounting, Timing Protocol Auditing, Runtime Routing Non-Leakage, and Hardware Boundary Scoping. We apply this verification procedure to an extensive empirical case study auditing 12 candidate neural network models across quantization, pruning, and distillation paradigms. Our independent audit uncovers and resolves 20 distinct numerical discrepancies between training-time reports and verified binary artifacts (with metric variances up to $7.82\%$). Furthermore, we demonstrate that evaluating gating thresholds directly on test data introduces an artificial $+1.8\%$ optimistic accuracy bias, proving the absolute necessity of split-isolated calibration. By providing rigorous verification formalisms and classifying evidence into clear empirical tiers, our framework establishes actionable protocols to support reproducible benchmarking and defensible deployment claims in edge AI research.
\end{abstract}
...
To address these challenges, this paper presents an independent, artifact-driven \textbf{TinyML Scientific Verification Framework}. We formalize verification protocols across seven dimensions of edge AI pipelines and demonstrate their efficacy through an exhaustive case study auditing 12 candidate models on the EngineFaultDB physical benchmark.
...
\begin{enumerate}
    \item \textbf{Evaluate Only Exported Binaries:} Always compute test metrics from the serialized \texttt{.tflite} or C-array header file, never from in-memory objects.
...
Our case study focused on tabular sensor classification. While convolutional and recurrent topologies exhibit different operator graphs, the principles of binary inspection, data isolation, and timing protocols generalize across all TinyML domains.

\section{Limitations}
The verification framework audits software artifacts and host simulations. Physical microcontroller execution requires on-device hardware profiling harnesses.
...
\section{Conclusion}
This paper presented an independent, multi-level verification framework for TinyML evaluation. By formalizing verification protocols across seven dimensions and auditing 12 candidate models, we demonstrated the necessity of inspecting serialized artifacts, resolving 20 numerical discrepancies and exposing sparsity-storage decoupling. Our framework establishes actionable protocols to support reproducible benchmarking in edge artificial intelligence.
