...
\title{QoS-Aware Multi-Fidelity Runtime for Real-Time Embedded AI under Dynamic Workload Contention}

\author{Antigravity Research Team%
\thanks{Manuscript received August 20, 2026. This research was conducted on the QoS-Aware TinyML Research Platform using the audited EngineFaultDB physical benchmark. All model binaries, runtime implementations, and simulation harnesses are fully reproducible.}%
}

% \markboth{IEEE Transactions on Computers / Real-Time Systems, August~2026}%
% {Antigravity Research Team: QoS-Aware Multi-Fidelity Runtime for Real-Time Embedded AI}

\IEEEtitleabstractindextext{%
\begin{abstract}
Deploying deep neural networks on resource-constrained microcontrollers (TinyML) is increasingly challenged by dynamic CPU contention, sporadic burst workloads, and strict inference deadlines. Static deployment paradigms force engineers into an inflexible compromise: either deploying an overly conservative lightweight model that perpetually sacrifices accuracy, or deploying a high-capacity model that suffers deadline violations under transient CPU contention. In this paper, we propose a \textbf{QoS-Aware Multi-Fidelity Runtime} that dynamically selects among pre-verified Pareto-optimal TinyML models based on execution deadlines, workload contention, and user-defined Quality of Service (QoS) policies. Operating strictly without ground-truth label access during execution, our runtime manages three operational modes: \texttt{FAST} (\texttt{student\_a\_8\_4\_fp32}, $160$ MACs), \texttt{BALANCED} (\texttt{pruned\_mlp\_14f\_75pct}, $96$ MACs), and \texttt{HIGH\_FIDELITY} (\texttt{student\_b\_16\_4\_fp32}, $304$ MACs). We formalize and evaluate four dynamic scheduling policies across an 80-configuration experimental sweep (5 deadlines $\times$ 4 workload contention regimes $\times$ 4 policies) evaluated over 11,200 held-out test frames. Our trace-driven simulation demonstrates that under high and burst contention, the \texttt{BALANCED} policy dynamically transitions from the dense model to the sparse model, achieving a \textbf{68.4\% reduction in theoretical active compute} ($96$ vs. $304$ MACs per inference) while matching or exceeding the diagnostic macro F1 score ($0.7563$ vs. $0.7390$) with only a $-0.37\%$ accuracy difference. Furthermore, controlled ablation studies prove that our QoS-aware runtime achieves a $+3.21\%$ accuracy improvement over static minimal execution and prevents starvation during CPU bursts, establishing dynamic multi-fidelity selection as an effective systems approach for edge intelligence.
\end{abstract}%
\begin{IEEEkeywords}
TinyML, Quality of Service, Dynamic Model Selection, Multi-Fidelity Runtime, Real-Time Embedded Systems, Microcontroller Deep Learning, Adaptive Edge AI.
\end{IEEEkeywords}%
}
...
To resolve this trade-off, we propose a \textbf{QoS-Aware Multi-Fidelity Runtime Architecture} for embedded TinyML. Rather than relying on a static model, our runtime system maintains a pre-verified registry of Pareto-optimal models spanning multiple operational modes (\texttt{FAST}, \texttt{BALANCED}, and \texttt{HIGH\_FIDELITY}). A lightweight, deadline-aware QoS scheduler dynamically assesses operational workload contention and deadline headroom, adaptively selecting the optimal model for each incoming observation vector.
...
\begin{itemize}
    \item \textbf{Ablation A (Static Best vs. QoS):} Proves that QoS-aware scheduling matches the accuracy of static execution while cutting active arithmetic operations by $68.4\%$ ($96$ vs. $304$ MACs) and boosting macro F1 ($0.7563$ vs. $0.7390$).
    \item \textbf{Ablation B (Static Minimal vs. QoS):} Proves that our runtime delivers a $+3.21\%$ accuracy gain over static lightweight deployment.
...
\section{Discussion}
\subsection{System-Level Trade-Offs in Edge Machine Learning}
Our results establish that multi-model dynamic scheduling provides a superior Pareto trade-off compared to monolithic static deployment. By decoupling model fidelity from static binary flashing, the runtime allows microcontrollers to achieve high fidelity when idle, while safely degrading compute load when foreground control routines demand CPU cycles.
...
While evaluated on automotive engine fault telemetry, the QoS scheduling policies and multi-fidelity runtime abstractions apply directly to general cyber-physical sensor streams.

\section{Limitations}
\begin{enumerate}
    \item \textbf{Synthetic Workload Multipliers:} Workload contention was simulated via synthetic scaling factors rather than physical concurrent RTOS threads.
    \item \textbf{Host Timing Granularity:} Latencies were measured on an x86\_64 host CPU; absolute sub-microsecond timings reflect host CPU performance rather than microcontroller clocks.
\end{enumerate}
...
\section{Conclusion}
This paper presented a QoS-Aware Multi-Fidelity Runtime for TinyML inference under dynamic workload contention. By adaptively selecting among verified Pareto-optimal models, our runtime reduces active computational load by $68.4\%$ under high contention while preserving diagnostic macro F1 ($0.7563$) and guaranteeing zero ground-truth leakage. These verified systems contributions establish dynamic multi-fidelity scheduling as an effective approach for robust, deadline-compliant edge intelligence.
