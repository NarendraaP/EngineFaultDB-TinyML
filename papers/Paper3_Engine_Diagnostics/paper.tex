...
\thanks{Manuscript received August 20, 2026. (Corresponding author: Narendra Satish, e-mail: narendresh.p@gmail.com). All model artifacts and evaluation pipelines are fully reproducible.}%
}

% \markboth{IEEE Transactions on Industrial Informatics, August~2026}%
% {Satish: Hierarchical Multi-Fidelity Inference for Engine Fault Diagnosis}
...
\caption{Hierarchical diagnostic policy sensitivity analysis across gating thresholds $\theta \in [0.00, 1.00]$: (a) Multi-class Diagnostic Accuracy, (b) Mode~B Trigger Rate, (c) Anomaly Recall, and (d) Accuracy vs. Compute Pareto Trade-off.}
...
\end{itemize}
This stability demonstrates that gating thresholds calibrated on validation data generalize to unseen distributions without threshold overfitting.
...
\begin{enumerate}
    ...
    \item \textbf{Controlled Dynamometer Dataset:} EngineFaultDB was collected under controlled steady-state dynamometer test-bench instrumentation. Evaluating performance on noisy, transient on-road drive cycles and dynamically shifting ambient temperatures remains an important subject for future physical deployment studies.
\end{enumerate}
